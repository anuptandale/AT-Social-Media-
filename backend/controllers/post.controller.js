...
export const getAllPosts = async (req, res) => {
    try {
        ...
        const posts = await Post.find().sort({ createdAt: -1 }).populate({ //we can use above line of code but in respose we dont want password so we are using this code
            path: "user",
            select: "-password",
        })
            .populate({
...
export const getFollowingPosts = async (req, res)=>{
    try {
        ...
        console.log("Error in getFollowings controller: ", error);
        res.status(500).json({error: "Internal server error"});
    }
}
